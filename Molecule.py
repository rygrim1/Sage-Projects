...
def bent_geometry(XY2,Pi_Key,Pi_Bond):
	...
	Central_Atom_Substitution = { A[0]: 'Xe' }

	Substituents = { S[1] : 'H', S[2] : 'H' }

	Additions = { S[1] : 'O', S[2] : 'N' }
...
def tetra(XY4,q):
	...
	Axis_label = { C3R[1]: 'C3', C2R[1]: 'C2' }

	Substituents = { S[1]: 'H', S[2]: 'H', S[3]: 'H', S[4]: 'H' }

	Additions = { S[1]: 'Cl', S[2]: 'B', S[3]: 'B', S[4]: 'Cl' }

	Central_Atom = { A[0] : 'Br' }

	Sub_Bonds = { 'A0-S1' : [A[0],S[1]], 'A0-S2' : [A[0],S[2]], 'A0-S3' : [A[0],S[3]], 'A0-S4': [A[0],S[4]] }

	Sym_Scale = { 'yz' : [(-1.5,1.5),(-2,2),(-1,1)], 'xy': [(-1.5,1.5),(-2,2),(-1,1)] }
	...
	if q == 3:
		...
		del (Substituents[S[1]], Sub_Bonds['A0-S1'], Axis_label[C2R[1]], C2R_pair[1], Sym_Planes[2])
...
def pi_planar(Ax,Ay):
	...
	if Ay[1] == 0.75:
		...
		PiBond = LineSegment(Ax1, Ay1, 1, color = 'black')
		PiBond += LineSegment(Ax2, Ay2, 1, color = 'black')
	...
	if Ay[1] == -0.75:
		...
		PiBond = LineSegment(Ax1, Ay1, 1, color = 'blue')
		PiBond += LineSegment(Ax2, Ay2, 1, color = 'blue')
...
#-------------------------------------------------------------------------------------------------------------------------
	...
	def Cyclobutadiene(Mol,A1,A2,A3,A4):
		...
		Subs_Cyclo = { Sub[1] : 'H', Sub[2]: 'H', Sub[3]: 'Cl', Sub[4]: 'H'}
...
#-------------------------------------------------------------------------------------------------------------------------
	...
	def Ethylene(Mol,A1,A2):
		...
		Subs_Ethene = { Sub[1] : 'B', Sub[2]: 'P', Sub[3]: 'Cl', Sub[4]: 'H'}
	...
	A = { 1: (-0.75, 0, 0), 2: (0.75, 0, 0), 4: (-0.75, 0, -1.50), 3: (0.75, 0, -1.50) }
...
def Ethane(Mol,Key):
	...
	if S2[1] != (-x, 2, -1):
		...
		del (Sym_Scale['xz'], C2R['C2'], C3R['C3'])
